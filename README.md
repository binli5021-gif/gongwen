...
![公文排版工具界面预览](public/gongwen-editor-preview.png)

## 一眼看懂

- **复制文本，直接生成！** — 把公文正文粘贴进来，自动识别结构并生成规范版式。
- **左边编辑，右边预览！** — 输入内容后实时查看 A4 页面效果，排版结果更直观。
- **导入文件，马上整理！** — 支持拖入或导入 `.docx`、`.txt` 文件，快速开始排版。
- **一键导出 Word！** — 预览确认后直接生成可继续编辑、打印和发送的 `.docx` 文件。
- **标题自动规范！** — 自动识别并统一一至四级标题、标点和层级。
- **格式不用反复调！** — 常用公文格式开箱即用，也可以按需要调整字体、字号、页边距和行距。
